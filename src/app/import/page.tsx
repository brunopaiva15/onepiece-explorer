...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title: dict.importer.metaTitle }
}
/**
  ...
  return (
    <main id="contenu" className="mx-auto max-w-3xl px-4 py-6">
      ...
      <div className="flex items-center gap-4">
        ...
        >
          <span className="cartouche !text-ink/70">{t.chapterCartouche}</span>
          <span className="chiffre text-4xl">{suggested}</span>
        </div>
        <div className="min-w-0">
          <h1 className="text-primary">{t.heroTitle}</h1>
          <p className="mt-1 text-sm text-secondary">
            {t.heroLead(existing.length === 0 ? null : highest)}
          </p>
        </div>
      </div>

      <SummaryForm suggestedNumber={suggested} locale={session.locale} />
            ...
            {existing.map((chapter) => (
              <li key={chapter.id} className="flex items-baseline gap-3 py-2.5">
                ...
                <span className="text-sm text-primary">
                  {chapter.title ?? <span className="text-muted">{t.untitled}</span>}
                </span>
