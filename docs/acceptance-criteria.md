...
- [ ] Le rattrapage de l'existant reste une commande à lancer, pas une réparation automatique : `pnpm mysteres:refermer` relit les questions déjà ouvertes contre les scènes publiées après elles. C'est un jugement de machine, enregistré comme tel et sans preuve citée — sa source est la scène, pas une case — `src/domains/review/mystery-sweep.ts`, `tests/review/mystery-sweep.test.ts`.
