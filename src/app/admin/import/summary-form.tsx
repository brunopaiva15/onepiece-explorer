    ...
    <form action={submit} className="mt-8 space-y-7">
      ...
      {state?.ok && state.chapterId && (
        ...
        >
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {state.runId && (
              <Link href={`/admin/runs/${state.runId}`} className="bouton bouton-primaire !py-1.5 !text-sm">
                Suivre le traitement
              </Link>
            )}
            <Link href={`/admin/chapitres/${state.chapterId}`} className="bouton !py-1.5 !text-sm">
              Voir le chapitre
            </Link>
          </div>
