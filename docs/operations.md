...
| Des chapitres publiés et vides, en série | Un traitement qui n'a rien extrait laissait la même file vide qu'une revue terminée : plus rien à trancher, donc « lu jusqu'au bout ». Corrigé — une carte décidée est désormais exigée, et une extraction qui refuse fait échouer l'étape | `repair:chapitres-vides` pour ceux déjà dans la bibliothèque : il les dépublie et les rend à la file, sans rien supprimer. Lisez son `dry-run` : « vide » veut dire aucune carte **et** aucun fait daté du chapitre |
| Le centre de revue répond 404 | Le traitement n'a produit aucune proposition, et la page cherchait le chapitre dans ses propositions | Corrigé : la page s'ouvre et montre la quarantaine. La note de l'étape d'extraction, sur `/admin/runs/<id>`, dit laquelle des deux causes — refus du modèle, ou preuves inancrables |
