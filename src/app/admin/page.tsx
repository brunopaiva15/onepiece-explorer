...
/**
 * The command deck — the workshop's front door, at `/admin`.
 *
 ...
 *
 * It used to be `/`, and share that URL with the visitor's landing page — one
 * file branching on `isOwner` twice, so the public site's first impression was
 * whatever was left after the workshop's needs were served. They are two
 * audiences with nothing in common: this one has a queue, the other has never
 * been here before. They are now two pages.
 */
...
export default async function AdminHomePage() {
  /*
   * requireOwner(), not getViewerSession().
   *
   * The proxy already refuses an anonymous request to anything under /admin,
   * but the proxy is a convenience layer and this page is the workshop's front
   * door. Asking for the owner's session here means the page cannot render for
   * anyone else even if the gate in front of it is misconfigured.
   */
  ...
  if (p.aRelire.length > 0) {
    taches.push(
      <Tache
        ...
        ton="or"
        href={`/admin/chapitres/${p.aRelire[0]!.id}`}
  ...
  if (p.enEchec.length > 0) {
    taches.push(
      <Tache
        ...
        ton="rouge"
        href={`/admin/chapitres/${p.enEchec[0]!.id}`}
  ...
  if (p.enCours.length > 0) {
    taches.push(
      <Tache
        ...
        ton="mer"
        href={`/admin/chapitres/${p.enCours[0]!.id}`}
        action="Suivre"
  ...
  if (p.aTraiter.length > 0) {
    taches.push(
      <Tache
        ...
        ton="mer"
        href={`/admin/chapitres/${p.aTraiter[0]!.id}`}
        action="Lancer"
    ...
    <main id="contenu" className="mx-auto max-w-6xl px-5 py-6">
      <section>
        ...
        {taches.length > 0 ? (
          ...
          >
            ...
            <Link href="/admin/import" className="bouton mt-3 !py-1 !text-sm">
      ...
      <section className="mt-8">
        ...
        <p className="mt-3 text-sm text-secondary">
          Les {p.publies.length} chapitres publiés sont ceux que le site public
          montre.{' '}
          <Link href="/" className="text-accent-strong underline underline-offset-2">
            Voir le site tel qu&apos;un visiteur le voit →
          </Link>
        </p>
      </section>
