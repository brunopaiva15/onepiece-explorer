...
function nextAction(chapter: ChapterSummary): { href: string; label: string; kind: string } {
  switch (chapter.status) {
    case 'draft':
    case 'uploaded':
      return { href: `/admin/chapitres/${chapter.id}`, label: 'Traiter', kind: 'bouton-primaire' }
    case 'processing':
      return { href: `/admin/chapitres/${chapter.id}`, label: 'Suivre', kind: 'bouton-mer' }
    case 'review':
      return { href: `/admin/chapitres/${chapter.id}`, label: 'Relire', kind: 'bouton-primaire' }
    case 'published':
      return { href: `/delta/${chapter.number}`, label: 'Le delta', kind: '' }
    default:
      return { href: `/admin/chapitres/${chapter.id}`, label: 'Ouvrir', kind: '' }
  }
}
